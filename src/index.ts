...
import { findCriticalPathsForNodes, findLongestUniquePaths, groupTopologicalSort } from './cycles.js'
...
const criticalPathOptimizer = async (db: sqlite3.Database, ioIdentifierCache: LimitedMap<string, Gate[] | null>, gates: Gate[], sliceSize: number, uniqueCriticalPaths: number[][]): Promise<{ changed: boolean, gates: Gate[] }> => {
	...
	for (const lineNumbers of uniqueCriticalPaths) {
		for (let a = 0; a < lineNumbers.length - sliceSize; a++) {
			...
			if (sliceSize > 2) {
				const replacement = findUselessVars(sliceGates, variableIndexMapping, false)
				if (replacement !== undefined) {
					const oldremoved = replace(gates, chosenLines.map((l) => ({ start: l, end: l, replacement: [] })))
					const newCircuit = insertArrayAtIndex(oldremoved, gateSimplifier(replacement, false), firstRow)
...
const optimize = async (db: sqlite3.Database, originalGates: Gate[], wires: number, problemName: string, verbose: boolean) => {
	...
	let optimizerState: 'MassOptimizer' | 'CriticalPathOptimizer' = 'MassOptimizer'
	...
	while (true) {
		...
		switch(optimizerState) {
			case 'MassOptimizer': {
				let sliceToUse = 2
				const MAX_SLICES = 20
				for (; sliceToUse < MAX_SLICES; sliceToUse++) {
			...
			case 'CriticalPathOptimizer': {
				...
				const criticalPaths = findCriticalPathsForNodes(dependencies)
				const uniqueCriticalPaths = findLongestUniquePaths(criticalPaths)
				for (let sliceToUse = 2; sliceToUse < 50; sliceToUse++) {
					const optimizationOutput = await criticalPathOptimizer(db, ioIdentifierCache, optimizedVersion, sliceToUse, uniqueCriticalPaths)
					if (optimizationOutput.changed) {
						optimizedVersion = optimizationOutput.gates
						break
					}
